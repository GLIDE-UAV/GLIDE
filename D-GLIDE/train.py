...
if __name__ == '__main__':
    ...
    class CustomPolicy(ActorCriticPolicy):
        def __init__(self, sess, ob_space, ac_space, n_env, n_steps, n_batch, reuse=False, **kwargs):
            ...
            num_agents=int(self.pdtype.size/3)
            with tf.variable_scope("model", reuse=reuse):
                ...
                for agent in range(num_agents):
                    with tf.variable_scope(f'agent_{agent}',reuse=reuse):
                        pi_h = extracted_features
                        for i, layer_size in enumerate([256,256]):
                            pi_h = activ(tf.layers.dense(pi_h, layer_size, name=f'pi_fc_{agent}' + str(i)))
                        pi_latent = pi_h
                        mean_agent = linear(pi_latent, f'mu_{agent}', 3, init_scale=0.01, init_bias=0.0)
                    means.append(mean_agent)
                mean=tf.concat(means,axis=1)
                logstd = tf.get_variable(name='logstd', shape=[1, self.pdtype.size], initializer=tf.zeros_initializer())
                ...
                with tf.variable_scope("value",reuse=reuse):
                    ...
                    vf_latent = vf_h
                    q_values = linear(vf_latent, 'q', self.pdtype.size, init_scale=0.01, init_bias=0.0)

                self._proba_distribution, self._policy, self.q_value = self.pdtype.proba_distribution_from_flat(pdparam), mean, q_values
